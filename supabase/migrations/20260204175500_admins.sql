UPDATE auth.users SET is_sso_user=TRUE;

ALTER TABLE public.contributors ADD COLUMN editor BOOLEAN NOT NULL DEFAULT FALSE;
...
CREATE POLICY "Editors may manage anyone's observation photos."
ON public.observation_photos FOR ALL
TO authenticated USING (
  EXISTS(
    SELECT true
    FROM public.contributors AS con
    JOIN public.user_contributor AS uc ON con.id = uc.contributor_id
    WHERE uc.user_uuid = (SELECT auth.uid()) AND con.editor
  )
);
...
CREATE POLICY "Editors may manage anyone's observations."
