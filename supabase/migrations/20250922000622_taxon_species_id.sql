CREATE FUNCTION inaturalist.species_id(taxon inaturalist.taxa) RETURNS INTEGER LANGUAGE SQL STABLE AS $$
SELECT CASE WHEN taxon.rank < 'species' THEN taxon.parent_id WHEN taxon.rank = 'species' THEN taxon.id ELSE NULL END;
$$;

ALTER TYPE taxon ADD ATTRIBUTE species_id INTEGER;
