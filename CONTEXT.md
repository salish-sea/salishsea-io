...
- **Curator** — a privileged human operator (currently the maintainer) who can force ingest actions: manually re-fetch or preview a specific source/window via the ingest endpoint. Distinct from a **Contributor** (who *observed* a record) and a **Shore regular** (an uptake persona). Ingest control, not observation.
