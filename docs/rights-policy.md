...
| Field | Source | Notes |
|-------|--------|-------|
| Geographic coverage | Stated bbox: the Acartia fetch extent (36–54°N, 120–136°W), with prose stating the two-part scope | Not derived — dataset *scope* is stated even when realized data omits edges. The box is the Southern Resident range because killer whales are held across all of it; every other Maplify/Whale Alert taxon is held only inside the Salish Sea + Strait of Juan de Fuca (the rule is applied to that source), and the `geographicDescription` must say so (decision [036](decisions/036-ingest-scope-killer-whales-range-wide.md)). |
...
| Taxonomic coverage | Stated as the Salish Sea marine-mammal remit — Cetacea, Pinnipedia, Lutrinae | Intentional taxonomic scope (decision [027](decisions/027-marine-mammal-scope-whale-centric-identity.md)). The realized `DISTINCT scientificName` list may be appended as a secondary block if useful, but the primary coverage is intentional, not realized. **The prose must state the SRC-01 gap** (gated by `verify-artifact` SC#4c): §2 excludes iNaturalist and HappyWhale, which hold nearly every pinniped and otter record we carry, so the realized archive is overwhelmingly cetacean and consumers seeking non-cetacean records should look to those publishers. Published `taxonomicClassification` names must resolve in the GBIF backbone where one exists: `Cetacea` (Order), `Phocidae` and `Otariidae` (Family). Never iNaturalist's `Phocoidea`, and not `Pinnipedia` — a GBIF synonym, kept to the prose (see 027). |
