...
*Implemented for individuals and ecotypes on 2026-09-11 ([PR #452](https://github.com/salish-sea/salishsea-io/pull/452)).* `/individuals/` and `/ecotypes/` serve the shape above; `/matrilines/` then still served the designation-keyed route [016](016-matriline-profile-pages.md) describes, until the note above. This paragraph and the two below record the sequencing as decided; the first sentence originally read "Nothing here is deployed yet."
