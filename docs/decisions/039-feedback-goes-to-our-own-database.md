...
**Each issue carries an invisible row marker**, and the notifier reads existing issues back before filing. The GitHub POST and the `notified_at` stamp are two operations, so a runner killed between them would otherwise leave a filed issue on an unstamped row and duplicate it on the next run. Listing by label rather than searching, because GitHub's search index lags by minutes — exactly the window this closes. The walk back stops at the oldest report in hand: an issue is always created after the row it reports, so nothing older can be ours, and the work is bounded by how long a report has been waiting rather than by how many we have ever filed. Only markers in issues the workflow itself wrote are trusted, since the label is applied by hand as often as by us and a body ending in a marker would otherwise let someone else's issue claim a row. Two notifiers running at once would defeat that check — both would list before either filed — so the script takes a Postgres session advisory lock, which covers a hand-run alongside a scheduled one and releases itself if the runner is killed.
