...
- **The "Everywhere" region now shows orcas beyond the Salish Sea and almost nothing else.** Previously it showed the whole Acartia box. [022](022-regions-filter-data.md) already defaulted the map to the Salish Sea, so nothing visible by default changes.
...
- **[CONTEXT.md](../../CONTEXT.md) and [PRODUCT.md](../../PRODUCT.md)** no longer say spatial scope "matches Acartia's boundaries"; they state the two-part rule.
- **iNaturalist and HappyWhale are untouched.** Their ingest queries are already scoped to the Salish Sea by taxon and place; this decision is about the one source we fetch from a wider box.
