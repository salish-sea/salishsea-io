...
**Status:** accepted · **Decided:** 2026-08-30 · **Amended:** 2026-09-20 — the ordering stands; the transitive wait on Q22 and the scoped-tagging fallback do not (see Consequences)
...
> **Amended 2026-09-20.** The first three consequences below are struck through. They were what this decision cost while the register lacked the finer group level — and the register now has it: [animals#31](https://github.com/salish-sea/animals/pull/31) mints the 73 Bigg's sub-lineages (`T073As` inside `T073s`) ahead of Q22 rather than waiting for its answer. That was a register decision, and its reasoning is recorded there, in animals ADR-0015: if the sub-lineages prove not to be real groups each merges into its parent lineage and consumers can follow that automatically, whereas tags written against a vocabulary that lacked them would stay coarse forever. Nothing was re-decided here; the hazard this record waited on went away. What holds now: **the migration and the first production tag no longer wait on Q22; the first production tag still waits on the migration (`salish-ox2.5`) and on the picker being built (`salish-8vr.9`).** Q22 stays open, and its answer confirms the sub-lineages or retires them.
