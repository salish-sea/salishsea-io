...
| [050](050-matriline-membership-is-the-registers.md) | Matriline membership is read from the register: a matriarch and all her descendants, so a matriarch is in her own matriline and her mother's, and a group mention reaches every living member (amends 016, answers part of `salish-ox2.5`) | accepted |
| [051](051-group-hierarchy-is-the-registers.md) | Whatever the register holds, we read from it; only what it lacks stays ours (matriarch, parentage, unpublished designations, nickname facts). The group hierarchy moves now (`group_parents`); sex, birth years and life status follow after reconciliation (`salish-ox2.8`) (extends 050) | accepted |
