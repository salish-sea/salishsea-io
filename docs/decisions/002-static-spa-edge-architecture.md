...
- **Supabase config baked into the edge bundle at synth** (2026-07-22, bd `salish-srg`; supersedes the SSM-parameter approach above): Lambda@Edge forbids env vars, but neither value is secret — the anon key ships in every browser bundle — and the deploy already passes it via `--context supabaseAnonKey`. Baking removes the SSM client, IAM grant, and a cross-region call from the 5s viewer-request cold-start budget. A synth without the context bakes empty values and the handler fails open. The handler also fires a fire-and-forget warmup fetch at module init: init runs at full CPU while the 128MB handler (the viewer-request hard cap) runs at ~1/13 vCPU, and the lazy undici load + DNS + TLS otherwise cost the first fetch per container ~2.5s (measured; the DB query itself is ~4ms).
