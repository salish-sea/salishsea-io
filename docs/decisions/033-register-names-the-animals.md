...
Migration `20260828100000` filtered `predicate_id = 'skos:exactMatch'` and gave as its reason that a broader match "would put a wider claim on the map than the data supports". The criterion that reason states — *does this predicate assert the same extension, or change it?* — admits more than one member. It stayed a list of one not because one was the right size, but because exactMatch was the only predicate the register then had that could be honoured.

**It remains an allow-list, and that is the part to preserve.** An allow-list refuses what it does not name, so `broadMatch`, `narrowMatch`, `relatedMatch` and every predicate the register invents after this are refused by the same mechanism, without anyone having to enumerate them. Failing closed is what forces the next such predicate to be adjudicated here rather than reaching a map pin unnoticed.

The tempting rewrite — `NOT IN ('skos:broadMatch', ...)` — reads like the same rule and is not. It admits `narrowMatch` and `relatedMatch` today and whatever arrives tomorrow. [`supabase/register-crosswalk.test.ts`](../../supabase/register-crosswalk.test.ts) fails against both that and the original one-member list, so neither drift can land quietly.
