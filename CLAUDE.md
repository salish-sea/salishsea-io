...
- Migrations: SELECT grants ship in the same migration that creates a table (Supabase RLS defaults silently zero out joins otherwise), and the migration's PR updates the pinned set in `supabase/read-grants.test.ts`, which CI checks against a fresh reset.
